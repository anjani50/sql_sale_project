...
order by idorder_details_id desc ;
insert order_details values ( 48621 , 4 , "hawaiian_m" , 5) ;

3.remove or delete the above values 
 delete from  order_details where   idorder_details_id = 48621 
 SELECT * FROM order_details
order by idorder_details_id desc  ;

4.change any name in the pizza types
 SELECT * FROM pizzapoint.pizza_types
alter table pizza_types rename column name to  name_type ;
 * change again to its original 
alter table pizza_types rename column name_type to  name;
